...
The catalogue is intended for downstream GIS/config-builder tooling. It answers:
  ...
  * Does the CLMS website publish an official CDSE legend image for the dataset,
    and what is the direct raw image URL behind its content page?
...
The script also probes the official CLMS /en/cdse-legends collection. These legend
".png" URLs are content pages rather than raw files, so when a matching page is
found the HTML is parsed for its /@@images/ asset and that direct image is verified.
Both the page URL and raw image URL are retained under style.legendImage. The image
is kept at dataset/style level because a single published image is not assumed to
apply to every advertised layer of a multi-layer service.
...
Official CLMS CDSE legend-image pages are also probed using the dataset identifier.
Their public *.png paths are HTML content pages, so the script extracts and verifies
the underlying /@@images/ image asset, retaining both URLs as style.legendImage.
...
CDSE_LEGEND_ROOT = "https://land.copernicus.eu/en/cdse-legends/"

TIMEOUT = 30
MAX_WORKERS = 8
USER_AGENT = "cdse-public-layer-discovery/2.3"
...
def cdse_legend_page_candidates(dataset_identifier):
    """
    Build conservative candidate filenames for the official CLMS CDSE legend
    collection.

    Current legend assets use names such as:
      dataset: swi_global_12.5km_10daily_v3
      page:    clms_global_swi_12-5km_v3_10daily.png

    A few ordering variants are included because the collection is editorial
    website content rather than a documented API. Only pages that actually
    resolve and contain a verified image asset are accepted.
    """
    match = re.fullmatch(
        r'(?P<product>.+?)_(?P<coverage>global|europe)_(?P<resolution>[^_]+)_(?P<frequency>[^_]+)_v(?P<version>\d+)',
        dataset_identifier,
        re.IGNORECASE,
    )
    if not match:
    ...
    coverage = match.group('coverage').lower()
    resolution = match.group('resolution').lower().replace('.', '-')
    frequency = match.group('frequency').lower()
    version = match.group('version')

    names = [
        f'clms_{coverage}_{product}_{resolution}_v{version}_{frequency}.png',
        f'clms_{coverage}_{product}_{resolution}_{frequency}_v{version}.png',
        f'clms_{coverage}_{product}_v{version}_{resolution}_{frequency}.png',
    ]

    if '_' in product:
        names.append(
            f'clms_{coverage}_{product.replace("_", "-")}_{resolution}_v{version}_{frequency}.png'
        )
    ...
    for name in names:
        if name in seen:
            continue
        seen.add(name)
        urls.append(urllib.parse.urljoin(CDSE_LEGEND_ROOT, name))
    return urls


def extract_raw_legend_image_url(page_url, page_html):
    """Extract the raw /@@images/ asset from a CLMS legend content page."""
    patterns = [
        r'''(?:href|src)=["']([^"']*(?:/@@images/|@@images/)[^"']+\.(?:png|jpe?g|gif|webp))(?:\?[^"']*)?["']''',
        r'''(?:href|src)=["']([^"']*(?:/%40%40images/|%40%40images/)[^"']+\.(?:png|jpe?g|gif|webp))(?:\?[^"']*)?["']''',
    ]
    for pattern in patterns:
        ...
        if match:
            ...
            page_basename = page_url.rstrip('/').rsplit('/', 1)[-1]
            if href.startswith(page_basename + '/@@images/'):
                page_parent = page_url.rstrip('/').rsplit('/', 1)[0] + '/'
                return urllib.parse.urljoin(page_parent, href)

            return urllib.parse.urljoin(page_url.rstrip('/') + '/', href)
    return None


def is_verified_image(body, headers):
    """Return True when an HTTP response is recognisably an image."""
...
def discover_cdse_legend_image(dataset_identifier):
    """
    Find an official CLMS CDSE legend image for a dataset when one is published.

    Returns only verified discoveries. A missing page is normal and does not
    create error/noise in the catalogue.
    """
    for candidate_url in cdse_legend_page_candidates(dataset_identifier):
        try:
            status, headers, body, final_page_url = fetch(
                candidate_url,
        ...
        if 'html' not in content_type:
            if is_verified_image(body, headers):
                return clean({
                    'source': 'official-clms-cdse-legend',
                    'pageUrl': final_page_url,
                    'imageUrl': final_page_url,
                    'contentType': content_type.split(';', 1)[0] or None,
                })
            continue

        page_html = body.decode('utf-8', errors='replace')
        image_url = extract_raw_legend_image_url(final_page_url, page_html)
        if not image_url:
            continue

        try:
            image_status, image_headers, image_body, final_image_url = fetch(
                image_url,
                accept='image/*,*/*',
            )
        except Exception:
        ...
        if image_status != 200 or not is_verified_image(image_body, image_headers):
            continue

        image_content_type = (
            image_headers.get('Content-Type') or image_headers.get('content-type') or ''
        ).split(';', 1)[0]
        return clean({
            'source': 'official-clms-cdse-legend',
            'pageUrl': final_page_url,
...
def discover_style_metadata(dataset_identifier, documentation_url, layers):
    ...
    official_legend_image = discover_cdse_legend_image(dataset_identifier)
    if official_legend_image:
        result["legendImage"] = official_legend_image
